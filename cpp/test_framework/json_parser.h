 ...
 public:
  ...
  template <
      ...
      typename std::enable_if<
          ...
          int>::type = 0>
  Json(const M &m) : Json(object(begin(m), end(m))) {}
  ...
  template <class V,
            typename std::enable_if<
                ...
                int>::type = 0>
  Json(const V &v) : Json(array(begin(v), end(v))) {}
   ...
   * field of
   * the given type. If not, return false and set err to a descriptive
   * message.
   */
...
// Internal class hierarchy - JsonValue objects are not exposed to users of
// this API.
class JsonValue {
 protected:
  friend class Json;
...
class Value : public JsonValue {
 protected:
  ...
  const T m_value;
  void dump(string &out) const override { json_parser::dump(m_value, out); }
};
...
const Json &JsonObject::operator[](const string &key) const {
  ...
  return (iter == end(m_value)) ? static_null() : iter->second;
}
...
struct JsonParser final {
  ...
  string parse_string() {
    ...
    while (true) {
      ...
      if (ch == 'u') {
        ...
        // JSON specifies that characters outside the BMP shall be encoded as
        // a pair
  ...
  Json parse_number() {
    ...
    if (str[i] != '.' && str[i] != 'e' && str[i] != 'E' &&
      ...
      return static_cast<int>(std::atoll(str.c_str() + start_pos));
    }
   ...
   * advance
   * the input and return result. If not, flag an error.
   */
  Json expect(const string &expected, Json result) {
    assert(i != 0);
    i--;
